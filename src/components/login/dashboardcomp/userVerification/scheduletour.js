...
class SceduleTournament extends React.Component {
  ...
  render() {
    ...
    const { eventData, userStatus, loading } = this.state;
    return (
      <React.Fragment>
        <CssBaseline />
        <div className={classes.root}>
          <Paper elavation={10} square={false}>
            {userStatus.length ? (
              <InfoComponent variant="h4" message={userStatus} />
            ) : (
                    ...
                    columns={[
                      {
                        ...
                        title: "Date",
                        render: (rowData) =>
                          moment(rowData.scheduleDate).format("l"),
                      },
                      {
                        field: "eventName",
                        title: "Event Name",
